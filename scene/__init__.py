...
class Scene:
    ...
    def getsTrainCameras(self, scale=1.0):
        return self.train_cameras[scale]

    def getTestCameras(self, scale=1.0):
        return self.test_cameras[scale]
    def getsTestCameras(self, scale=1.0):
        return self.test_cameras[scale]
    def getsAllCameras(self, scale=1.0):
        all_camercas = self.train_cameras[scale].append(self.test_cameras[scale])
        return all_camercas[scale]

class LargeScene(Scene):
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, load_vq=False, shuffle=True,
                 resolution_scales=[1.0]):
        """b
        :param path: Path to colmap scene main folder.
        """
        super().__init__(args, gaussians, load_iteration, shuffle, resolution_scales)
        self.model_path = args.model_path
        self.loaded_iter = None
        self.load_vq = load_vq
        self.gaussians = gaussians
        self.pretrain_path = args.pretrain_path
        self.partition_name = args.source_path.split('/')[-1]
        if load_iteration:
            if load_iteration == -1:
                self.loaded_iter = searchForMaxIteration(os.path.join(self.model_path, "point_cloud"))
            else:
        ...
        self.stest_cameras = {}
        if args.block_id >= 0:
            partition = np.load(os.path.join(args.source_path, "data_partitions", f"{self.partition_name}.npy"))[:,
                        args.block_id]
            if args.aabb is None:
                args.aabb = np.load(
                    os.path.join(args.source_path, "data_partitions", "aabb.npy")).tolist()
            print(f"Using Partition File {self.partition_name}.npy")
        else:
            partition = None
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.llffhold,
        ...
        for resolution_scale in resolution_scales:
            print("Loading Training Cameras")
            self.strain_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale,
                                                                             args)
            print("Loading Test Cameras")
            self.stest_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras, resolution_scale,
                                                                            args)
        # print(f"scene_info是{len(scene_info.train_cameras)}")
        # print(f"self.strain_cameras是{len(self.strain_cameras)}")
        # print(f"self.train_cameras是{len(self.train_cameras)}")
        if self.load_vq:
            ...
            if args.add_background_sphere:
                ...
                background_sphere_point_xyz = (
                                                      unit_sphere_points * scene_radius * args.background_sphere_radius) + scene_center
                background_sphere_point_rgb = np.asarray(np.random.random(background_sphere_point_xyz.shape),
                                                         dtype=np.float64)
                ...
                print("added {} background sphere points, rescale prune extent from {} to {}".format(n_points,
                                                                                                     scene_radius,
                                                                                                     self.cameras_extent))
    ...
    def save(self, iteration, args=None):
        ...
        if args.block_id >= 0:
            ...
            block_point_cloud_path = os.path.join(self.model_path,
                                                  "point_cloud_blocks/scale_1.0/iteration_{}".format(iteration))
    ...
    def getsTrainCameras(self, scale=1.0):
        return self.strain_cameras[scale]

    def getTestCameras(self, scale=1.0):
        return self.test_cameras

    def getsTestCameras(self, scale=1.0):
        return self.stest_cameras[scale]
    def getsAllCameras(self, scale=1.0):
        all_camercas = self.train_cameras + self.test_cameras
        return all_camercas
